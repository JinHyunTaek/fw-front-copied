...
void main() {
  ...
  setUp(() {
    ...
    userModelWithoutNickname = UserModel(
      ...
      earnedBetSucceedPoint: userModel.earnedBetSucceedPoint,
      id: userModel.id,
      nickname: null,
  ...
  test(
    ...
    () async {
      ...
      final expectedUserModel = container.read(userProvider);
      expect(
        (expectedUserModel as UserLoginErrorErrorModel).message,
        equals(UserModelErrorMessage.LOGIN_FAILURE_USER_NULL.message),
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      when(
        ...
        DioException(
          ...
          response: Response(
            ...
            requestOptions: requestOptions,
            data: {
              "errorCode": UserModelErrorMessage.DUPLICATED_EMAIL_403.name,
            },
          ),
        ),
      );
      ...
      expect(
        (expectedUserModel as UserLoginErrorErrorModel).message,
        equals(UserModelErrorMessage.DUPLICATED_EMAIL_403.message),
      );
    },
  );
  ...
  test('join() success case', () async {
    ...
    // then
    final expectedUserModel = container.read(userProvider);
    expect(expectedUserModel, isA<UserModelJoining>());
  });
  ...
  test(
    ...
    () async {
      ...
      when(() => userRepo.join(request: any(named: 'request'))).thenThrow(
        DioException(
          ...
          response: Response(
            statusCode: 400,
            requestOptions: requestOptions,
            data: { "errorCode" : UserModelErrorMessage.LOGIN_FAILURE_500.name},
          ),
        ),
      );
      ...
      expect(
        (expectedUserModel as UserLoginErrorErrorModel).message,
        equals(UserModelErrorMessage.LOGIN_FAILURE_500.message),
      );
    },
  );
}
