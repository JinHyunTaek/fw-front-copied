...
class JoinCodeVerificationScreen extends ConsumerStatefulWidget {
  final bool isJoin;
  final String email;

  const JoinCodeVerificationScreen({
    required this.email,
  ...
  @override
  ConsumerState<JoinCodeVerificationScreen> createState() =>
      _JoinCodeVerificationScreenState();
}

class _JoinCodeVerificationScreenState
    extends ConsumerState<JoinCodeVerificationScreen> {
  Timer? timer;
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: Column(
                    children: [
                      Text(
                        '${widget.email}으로 인증 코드를 전송하였습니다.',
                    ...
                    onPressed:
                        isCodeEmpty() || !isCodeCorrect
                            ...
                            : () async {
                              ...
                              /// 여기서부터는 join or (not join & incorrect Code)
                              final state = ref.read(smtpProvider);
                              if (state == SmtpStatus.incorrectCode) {
                                setState(() {
                                  isCodeCorrect = false;
                                });
                              } else {
                                if (state == SmtpStatus.unknownError) {
                                  Navigator.of(context).pop();
                                } else {
                                  Navigator.of(context).pushReplacement(
                                    MaterialPageRoute(
                                      builder: (_) {
                                        return EndJoinScreen(
                                          email: widget.email,
                                        );
                                      },
                                    ),
